...
async function takeStudentSnapshot(studentId) {
  ...
  statsData.forEach((st,i)=>{
    ...
    ctx.fillStyle='rgba(255,255,255,0.07)'; ctx.strokeStyle='rgba(255,255,255,0.1)'; ctx.lineWidth=1;
    ctx.beginPath(); ctx.roundRect(sx,sy,statW,72,12); ctx.fill(); ctx.stroke();
    ctx.font='20px serif'; ctx.textAlign='center'; ctx.fillText(st.emoji,sx+statW/2,sy+24);
    ctx.font='bold 24px Arial,sans-serif'; ctx.fillStyle='#ffffff';
    ctx.fillText(st.val,sx+statW/2,sy+48);
    ctx.font='10px Arial,sans-serif'; ctx.fillStyle='rgba(255,255,255,0.4)';
    ctx.fillText(st.key,sx+statW/2,sy+64); ctx.textAlign='left';
  });
  ...
  if(achs.length>0){
    achs.slice(0,10).forEach((a,i)=>{
      ...
      ctx.fillStyle='rgba(255,255,255,0.18)'; ctx.strokeStyle='rgba(255,255,255,0.35)'; ctx.lineWidth=1.5;
